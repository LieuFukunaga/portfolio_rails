class List < ApplicationRecord
  ...
  def self.split_keyword(search)
    ...
    unduplicated_word = split_keyword - unduplicated_first
    split_keyword = unduplicated_word + unduplicated_first
  end
  ...
  def self.list_search(search, user_id)
    if search.present?
      ...
      lists = lists.group_by{ |e| e }.select{ |key, value| value.size >= keyword_amount }.map(&:first)   # キーワードすべてに部分一致している検索結果を配列化
